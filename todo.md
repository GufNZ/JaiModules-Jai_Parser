...
Remaining `how_to` first-error leads (2026-09-25; 73/84 zero-diagnostic, 11 recovering files, 70 diagnostics).
...
- [ ] Parse comma-separated compound assignments to arbitrary LValues such as `array[it], array[it+1] += get_u8s()`: `010_calling_procedures.jai` (first error at line 224, expected `;` at `,`).
- [ ] Parse value-form `ifx` without an explicit `else`, such as `mame := ifx thing then thing.name;`: `025_ifx.jai` (line 160, expected `else` at `;`).
- [ ] Investigate the call containing a block-bodied quick lambda, `call_with(5, x => { ... });`: `050_this.jai` (line 140, expected `;` at `(`).
- [ ] Parse expression-form `#ifx` with value branches: `095_static_if.jai` (line 270, expected an expression at `#`).
- [ ] Parse the `inline procedure_call(...)` statement inside a static branch: `115_auto_bakes.jai` (line 279, expected an expression at `inline`).
- [ ] Parse source-spelled `#no_reset` before declarations at module scope: `170_modify.jai`, `560_no_reset.jai` (first errors at lines 388 and 72, both unexpected `#`).
- [ ] Investigate the `#insert -> Code { ... }` short form and its statement boundary: `600_insert.jai` (line 419, expected `;` at `#`).
- [ ] Clear the controlled `<=` for-expansion first error by completing the controlled-flag grammar and fixtures in section 13.3: `730_for_expansions.jai` (line 362, expected an expression at `<=`).
- [ ] Parse `using,map(prefix_with_gl) procs: Procs;` without silently consuming an unsupported modifier: `044_using_advanced/main.jai` (line 428, unexpected identifier).
- [ ] Parse the file-scope `#scope_module` directive: `380_module_parameters/local_modules/Rendering.jai` (line 62, unexpected `#`).
- [ ] Re-triage each guide after its first error is fixed, record any newly exposed valid source forms as separate tasks, and keep the compatibility completion checkbox above open until all valid guides parse completely with zero diagnostics and the required fixtures pass.
